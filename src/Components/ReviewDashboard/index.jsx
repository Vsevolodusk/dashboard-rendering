...
class DashBoard extends Component {
  ...
  render() {
    ...
    const userArray = usersData.map((userData) => (
      <Review key={userData.id} userData={userData} />
    ));
    const highMarkArray = usersData.map((userData) => (
      <HighReviews key={userData.id} userData={userData} />
    ));
    return (
      <div className={styles.userReviewsContainer}>
        <div className={styles.reviews}>{userArray}</div>
        <div className={styles.reviews}>{highMarkArray}</div>
      </div>
    );
  }
}
